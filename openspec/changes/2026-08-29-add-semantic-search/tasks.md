...
- [ ] 2.1 `lib/scrapers/types.ts` の記事型に `bodyText?: string` を追加する
- [ ] 2.2 `lib/scrapers/qiita.ts` で API v2 レスポンスの `body` を取得・格納する
- [ ] 2.3 `app/api/scrape/route.ts` の upsert に `bodyText` を含める
- [ ] 2.4 既存の Jest テストを更新し、本文取得のテストケースを追加する
...
- [ ] 4.1 `POST /search` を実装する
  - クエリ文字列を埋め込み化
  - pgvector のコサイン距離（`<=>`）で近傍検索
  - 記事 ID 配列とスコアを返す（上位 N 件、N はパラメータ化）
- [ ] 4.2 類似度のカットオフ閾値を設ける（関連性の低い記事を除外）
- [ ] 4.3 API キーによる簡易認証ミドルウェアを実装する
- [ ] 4.4 CORS 設定（本番は Vercel ドメインのみ許可）
- [ ] 4.5 pytest で検索ロジックのテストを書く
- [ ] 4.6 ローカルで検索が意図通り動くことを確認する（タイトルに含まれない語で検索）
